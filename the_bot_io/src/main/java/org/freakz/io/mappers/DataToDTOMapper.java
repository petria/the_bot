...
public class DataToDTOMapper {
  ...
  private BotConnectionResponse toBotConnectionResponse(BotConnection botConnection) {
    ...
    botConnection.getChannelMap().values().stream()
        .map(this::toBotConnectionChannelResponse)
        .forEach(channel -> response.getChannels().add(channel));
  ...
  private BotConnectionChannelResponse toBotConnectionChannelResponse(
    ...
    response.setConfigured(botConnectionChannel.isConfigured());
    return response;
  }
}
