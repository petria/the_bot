...
public class RestMessageSendClient {
  ...
  public ResponseEntity<String> stopProcessingIndicator(int connectionId, Message message) {
    String url = baseUrl + "/processing/stop/" + connectionId;
    try {
      return restTemplate.postForEntity(url, message, String.class);
    } catch (Exception e) {
      log.debug("Error stopping processing indicator: {}", e.getMessage());
      return ResponseEntity.internalServerError().body(e.getMessage());
    }
  }
