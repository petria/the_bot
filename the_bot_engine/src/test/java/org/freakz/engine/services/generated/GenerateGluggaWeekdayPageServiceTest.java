...
class GenerateGluggaWeekdayPageServiceTest {

  @TempDir
  Path tempDir;

  @Test
  void generatesWeekdaySummaryAndNickBestDays() throws Exception {
    ...
    when(dataValuesService.getRawDataValues("#hokandev", "ircnet", "GLUGGA_COUNT"))
        .thenReturn(List.of(
            new DataValues("NickA", "ircnet", "#hokandev", "GLUGGA_COUNT_2026_11_05_10", "3"),
            new DataValues("NickA", "ircnet", "#hokandev", "GLUGGA_COUNT_2026_12_05_10", "1"),
            new DataValues("NickB", "ircnet", "#hokandev", "GLUGGA_COUNT_2026_12_05_11", "6"),
            new DataValues("NickC", "ircnet", "#hokandev", "GLUGGA_COUNT_bad", "9")));
    ...
    GeneratedPageResponse response = new GenerateGluggaWeekdayPageService().handleServiceRequest(request);
    ...
    assertThat(props.get("totalCount")).isEqualTo(10);
    assertThat(props.get("skippedRowCount")).isEqualTo(1);
    assertThat((List<?>) props.get("daySummary"))
        .anySatisfy(row -> {
          Map<?, ?> item = (Map<?, ?>) row;
          assertThat(item.get("day")).isEqualTo("Mon");
          assertThat(item.get("count")).isEqualTo(3);
        })
        .anySatisfy(row -> {
          Map<?, ?> item = (Map<?, ?>) row;
          assertThat(item.get("day")).isEqualTo("Tue");
          assertThat(item.get("count")).isEqualTo(7);
        });
    assertThat((List<?>) props.get("nickRows"))
        .anySatisfy(row -> {
          Map<?, ?> item = (Map<?, ?>) row;
        ...
        .anySatisfy(row -> {
          Map<?, ?> item = (Map<?, ?>) row;
          assertThat(item.get("nick")).isEqualTo("NickB");
          assertThat(item.get("bestDay")).isEqualTo("Tue");
          assertThat(item.get("bestDayCount")).isEqualTo(6);
        });
  }
