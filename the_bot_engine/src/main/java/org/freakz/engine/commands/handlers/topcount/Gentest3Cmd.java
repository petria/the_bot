...
@HokanCommandHandler
public class Gentest3Cmd extends AbstractCmd {

  @Override
  public void initCommandOptions(JSAP jsap) throws JSAPException {
    jsap.setHelp("Generate a temporary web page with longest GLUGGA_COUNT calendar-day streaks.");
  ...
  public String executeCommand(EngineRequest engineRequest, JSAPResult results) {
    GeneratedPageResponse response =
        doServiceRequest(engineRequest, results, ServiceRequestType.GenerateGluggaStreakPage);
    if (response == null || response.getUrl() == null || !"OK".equals(response.getStatus())) {
      return response == null ? "Could not generate glugga streak page." : response.getStatus();
    }
    return String.format(
        "Glugga streak page: %s (%d nicks, expires in 7 days)",
        response.getUrl(),
