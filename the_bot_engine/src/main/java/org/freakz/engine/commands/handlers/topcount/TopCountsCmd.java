...
public class TopCountsCmd extends AbstractCmd {
  ...
  public void initCommandOptions(JSAP jsap) throws JSAPException {
    ...
    jsap.setHelp(
        "Channel top key word counts. TopKey parameter need to one of: "
            + String.join(", ", TopCountsEnum.getPrettyNames()));

    UnflaggedOption uflg = new UnflaggedOption(ARG_TOP_KEY).setRequired(false).setGreedy(false);
    jsap.registerParameter(uflg);

    uflg = new UnflaggedOption(ARG_CHANNEL).setRequired(false).setGreedy(false);
    jsap.registerParameter(uflg);
  }
  ...
  public String executeCommand(EngineRequest engineRequest, JSAPResult results) {
    ...
    TopCountsResponse response =
        doServiceRequest(engineRequest, results, ServiceRequestType.GetTopCountsService);
    List<DataValuesModel> dataValues = response.getDataValues();
    if (dataValues.size() > 0) {
      int c = 1;
      String starredKey =
          String.format(" *%s*: ", dataValues.get(0).getKeyName().split("_")[0].toLowerCase());
