...
public class TopStatsCmd extends AbstractCmd {
  ...
  private static final Logger log = LoggerFactory.getLogger(TopStatsCmd.class);

  @Override
  public void initCommandOptions(JSAP jsap) throws JSAPException {

    jsap.setHelp(
        "Stats about counted thing, TopKey parameter need to one of: "
            + String.join(", ", TopCountsEnum.getPrettyNames()));

    UnflaggedOption opt =
        new UnflaggedOption(ARG_NICK).setDefault("me").setRequired(false).setGreedy(false);
    jsap.registerParameter(opt);

    UnflaggedOption uflg =
        new UnflaggedOption(ARG_TOP_KEY).setDefault("glugga").setRequired(false).setGreedy(false);
    jsap.registerParameter(uflg);

    opt =
        new UnflaggedOption(ARG_CHANNEL).setDefault("current").setRequired(false).setGreedy(false);
    jsap.registerParameter(opt);

    opt =
        new UnflaggedOption(ARG_NETWORK).setDefault("current").setRequired(false).setGreedy(false);
    jsap.registerParameter(opt);
  }
  ...
  private String mustBeError() {
    return String.format(
        "%s\n  TopKey must be one of: %s",
        getJsap().getUsage(), String.join(", ", TopCountsEnum.getPrettyNames()));
  }
  ...
  public String executeCommand(EngineRequest engineRequest, JSAPResult results) {
    ...
    TopStatsResponse response =
        doServiceRequestMethods(engineRequest, results, ServiceRequestType.GetTopStatsRequest);

    StatsNode statsNode = response.getNodeMap().get(nick.toLowerCase());
    //        List<StatsNode> collect = nodeMap.values().stream().sorted(Comparator.comparing(o ->
    // o.statDaysPercent, Comparator.reverseOrder())).toList();
    if (statsNode != null) {
      ...
      sb.append(
          String.format(
              "       plus  streak: %s - %s = %d days\n",
              plusStreakStartDay, plusStreakEndDay, statsNode.plusStreakDays));
      sb.append(
          String.format(
              "       minus streak: %s - %s = %d days\n",
              minusStreakStartDay, minusStreakEndDay, statsNode.minusStreakDays));
      sb.append(
          String.format(
              " days used/not used: %d - %d = %f %%\n",
              statsNode.statsDays, statsNode.totalDays, percent));

      //            String streakStart = String.format("%02d.%02d.%4d",
      // statsNode.plusStreakStart.getDayOfMonth(), statsNode.firstStatDay.getMonthValue(),
      // statsNode.firstStatDay.getYear());
      //            String stats = String.format("%s :: %s stats starting from %s days used/not used
      // %d/%d ratio: %f %% - longest %s spree: %d - longest NO %s spree: %d days",
      // engineRequest.getFromSender(), topKey, firstStatDay, statsNode.statsDays,
      // statsNode.totalDays, percent, topKey, statsNode.plusStreakDays, topKey,
      // statsNode.minusStreakDays);
      return sb.toString();
