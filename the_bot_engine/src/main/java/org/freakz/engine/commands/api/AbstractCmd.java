...
public abstract class AbstractCmd implements HokanCmd {
  ...
  @Getter private JSAP jsap = new JSAP();

  @Getter private BotEngine botEngine;
  ...
  public void initCommandOptions(JSAP jsap) throws NotImplementedException, JSAPException {
    throw new NotImplementedException(
        "Command handler must Override initCommandOptions(): " + getClass().getSimpleName());
  }
  ...
  public <T extends ServiceResponse> T doServiceRequest(
      EngineRequest engineRequest, JSAPResult results, ServiceRequestType serviceRequestType) {
    ServiceRequest request =
        ServiceRequest.builder().engineRequest(engineRequest).results(results).build();
    return botEngine.getHokanServices().doServiceRequest(request, serviceRequestType);
  }

  public <T extends ServiceResponse> T doServiceRequestMethods(
      EngineRequest engineRequest, JSAPResult results, ServiceRequestType serviceRequestType) {
    ServiceRequest request =
        ServiceRequest.builder().engineRequest(engineRequest).results(results).build();
