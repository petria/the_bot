...
public class DataValuesServiceImpl implements DataValuesService, DataSavingService {
  ...
  private Map<String, DataValuesModel> combineCounters(List<DataValues> modelsList, String key) {
    ...
    for (DataValues v : modelsList) {
      String mapKey =
          String.format(
              "%s_%s_%s",
              v.getNick().toLowerCase(),
              v.getNetwork().toLowerCase(),
              v.getChannel().toLowerCase());

      DataValuesModel n = combinedMap.get(mapKey);
      if (n == null) {
        n =
            new DataValuesModel(
                v.getNick(), v.getNetwork(), v.getChannel(), key, "" + v.getValue());
  ...
  //    @Override
  //    public List<DataValues> findAllByNickAndChannelAndNetworkAndKeyNameIsLike(String nick,
  // String channel, String network, String keyLike) {
  //        return dataValuesRepository.findAllByNickAndChannelAndNetworkAndKeyNameIsLike(nick,
  // channel, network, keyLike);
  //    }

  @Override
  //    @Transactional(readOnly = true)
  public DataValueStatsModel getValueStats(
      String nick, String channel, String network, String key) {
    DataValueStatsModel stats = new DataValueStatsModel();
    String[] days = {"Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"};

    String keyLike = key + ".*";
    List<DataValues> modelsList =
        dataValuesRepository.findAllByNickAndChannelAndNetworkAndKeyNameIsLike(
            nick, channel, network, keyLike);
    Map<String, Integer> dayCounts = new HashMap<>();
    for (String day : days) {
      dayCounts.put(day, 0);
    }
    Map<Integer, Integer> hourCounts = new HashMap<>();
    IntStream.rangeClosed(0, 23).forEach(i -> hourCounts.put(i, 0));
    for (String day : days) {
      dayCounts.put(day, 0);
    }
  ...
  public List<DataValuesModel> getDataValues(String channel, String network, String key) {
    String keyLike = key + ".*";
    List<DataValues> modelsList =
        dataValuesRepository.findAllByChannelAndNetworkAndKeyNameIsLike(channel, network, keyLike);
  ...
  public List<DataValuesModel> getDataValuesAsc(String channel, String network, String key) {
    ...
    Comparator<? super DataValuesModel> comparator =
        (Comparator<DataValuesModel>)
            (o1, o2) -> {
  ...
  public String getValue(String nick, String channel, String network, String key) {
    String value = null;
    DataValues data =
        dataValuesRepository.findByNickAndChannelAndNetworkAndKeyName(nick, channel, network, key);
    if (data != null) {
      value = data.getValue();
    }
    return value;
