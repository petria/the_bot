...
public class KeliKameratService extends AbstractService {
  ...
  public KelikameratWeatherData updateKelikameratWeatherData(KelikameratUrl url) {
    ...
    titleText =
        titleText.replaceFirst("Kelikamerat - ", "").replaceFirst("\\| Kelikamerat", "").trim();
  ...
  @Override
  public <T extends ServiceResponse> KelikameratResponse handleServiceRequest(
      ServiceRequest request) {

    log.debug("Handle request: {}", request);

    KelikameratResponse response = KelikameratResponse.builder().build();

    if (!isFirstUpdateDone.get()) {
      response.setStatus(
          String.format(
              "NOK: Initial data fetch still in progress: %d / %d", updateDone, toUpdate));
    } else {
