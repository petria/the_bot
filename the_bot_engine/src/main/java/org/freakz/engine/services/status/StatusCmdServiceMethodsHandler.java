...
public class StatusCmdServiceMethodsHandler extends AbstractService {
  ...
  public <T extends ServiceResponse> ServiceResponse handleServiceRequest(ServiceRequest request) {
    ...
    StatusReportService statusReportService =
        request.getApplicationContext().getBean(StatusReportService.class);
    ...
    for (StatusReportRequest value : values) {
      ...
      if (value.getName().equals("BOT_ENGINE") || current - value.getTimestamp() < 2000) {
        ...
        formattedValuesList.add(
            formatStatusReportRequest(value, timeDiffService, callCounts, verbose));
      }
    }
  ...
  private int[] getModuleCallCounts(
      StatusReportRequest value, ConcurrentMap<String, Integer> callCounts) {
    int in = 0;
    int inStatus = 0;
    int out = 0;
    int outStatus = 0;
  ...
  private String formatStatusReportRequest(
      ...
      TimeDifferenceService timeDiffService,
      int[] callCounts,
      boolean verbose) {
    long[] diffs =
        timeDiffService
            .getTimeDifference(statusReportRequest.getUptimeStart(), System.currentTimeMillis())
            .getDiffs();
    String who;
    ...
    if (verbose) {
      return String.format(
          "%12s - %-12s - %02d:%02d:%02d - in: %6d out: %6d -- i: %6d o: %6d",
          who,
          statusReportRequest.getName(),
          diffs[2],
          diffs[1],
          diffs[0],
          callCounts[0],
    ...
    return String.format(
        "%12s - %-12s - %02d:%02d:%02d - in: %6d out: %6d",
        who,
        statusReportRequest.getName(),
        diffs[2],
        diffs[1],
