...
public class ForecaWeatherService extends AbstractSpringService {
  ...
  private static final Logger log = LoggerFactory.getLogger(ForecaWeatherService.class);

  private static final ForecaDataFetcher forecaDataFetcher = new ForecaDataFetcher();
  ...
  public List<CountryCityLink> getMatchingCountryCityLinks(String place) {
    ...
    for (String cityKey : forecaDataFetcher.getToCollectLinks().keySet()) {
      ...
      if (countryCityLink.city.toLowerCase().matches(place)
          || countryCityLink.city2.toLowerCase().matches(place)) {
        ...
        if ((region.contains(pieces[0]) || country.contains(pieces[0]))
            && (city.matches(pieces[1]) || city2.matches(pieces[1]))) {
          matching.add(countryCityLink);
        }
      }
    }
  ...
  public <T extends ServiceResponse> CmpWeatherResponse handleCmpCmdServiceRequest(
    ...
    if (forecaDataFetcher.getToCollectLinks() == null) {
      response.setStatus(
          String.format("NOK: Initial data fetch still in progress: %d / %d", 0, -1));
    } else {
      ...
      for (String place : places) {
        ...
        for (CountryCityLink match : matching) {
          try {
            ...
            List<ForecaWeatherData> forecaWeatherData =
                forecaDataFetcher.fetchCityWeather(match.city, match.cityUrl, sunUpDown);
  ...
  public <T extends ServiceResponse> ForecaResponse handleForecaCmdServiceRequest(
      ServiceRequest request) {
    ForecaResponse response = ForecaResponse.builder().build();
