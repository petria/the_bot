...
The same `dev.properties` file also carries engine runtime settings such as Hermes, weather and signed context-token configuration. Optional placeholders can use `${ENV_NAME:default}`. Required placeholders use `${ENV_NAME}` and fail fast when missing.
...
Secrets and infrastructure values such as `HOKAN_NODE_CONTEXT_SECRET` are not runtime-mutable. They must come from CI/deploy environment, local IDEA environment, or the selected bootstrap properties file. Do not use `*.secret.properties` files for new config.

Minimum IDEA environment variables for local bot testing:

`BOT_CONFIG_FILE=./runtime/dev.properties`
`HOKAN_NODE_CONTEXT_SECRET`
`WEATHER_API_KEY`
`DISCORD_SECRET`
`TELEGRAM_SECRET`
...
`prod.properties` is the container-oriented example of the same common config shape. It uses container paths such as `/runtime` after the file is mounted by Docker Compose.
